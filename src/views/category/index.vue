		...
		<el-card shadow="never" class="toolbar-card">
			<div class="toolbar">
				...
				<div class="toolbar-right">
					<span class="total-info">共 {{ categoryStore.total }} 个分类</span>
				</div>
			</div>
		</el-card>
			...
			>
				...
				<el-form-item label="分类别名" prop="slug">
					<el-input
						...
						placeholder="URL 友好别名，如 frontend"
						maxlength="50"
					/>
				</el-form-item>
...
// 根据名称自动生成 slug（简易拼音替代：取名称的小写+连字符）
...
const handleSubmit = async () => {
	...
	const success = isEditMode.value
		? await categoryStore.update(data)
...
const handleDelete = async (row) => {
	const success = await categoryStore.remove(row.id);
	if (success) {
		handleSearch();
	}
};

// 每行独立的 loading 状态（ref 包裹确保响应式）
const statusLoadingMap = ref({});

const handleStatusChange = async (row) => {
	const previousStatus = row.status === 1 ? 0 : 1;
	
	// 使用 Vue.set 方式确保响应式更新
	statusLoadingMap.value = { ...statusLoadingMap.value, [row.id]: true };
	
	try {
		await categoryStore.toggleStatus(row.id, row.status);
		ElMessage.success("状态更新成功");
		
		// 显式更新表格数据，触发视图刷新
		const index = categoryStore.list.findIndex(item => item.id === row.id);
		if (index !== -1) {
			categoryStore.list[index].status = row.status;
		}
	} catch {
		// 失败时恢复原状态
