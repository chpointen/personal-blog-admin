 ...
 */
const handleLogin = () => {
  // 第一步：Element Plus 表单验证
	loginFromRef.value.validate(valid => {
		if (!valid) return; // 验证失败，直接返回
		console.log(loginForm.value);
    // 第二步：比较用户输入与获取的验证码是否一致
		if (loginForm.value.captcha_code !== codeNet.value) {
			ElMessage.error("验证码错误！");
			return;
		}
    // 第三步：调用 Pinia 登录 action
		loading.value = true;
    // 实际项目中，登录接口应该由后端提供，前端调用后端接口进行登录验证，这里为了演示方便，直接在前端进行登录验证
		// 验证码正确，继续登录流程
...
const getCodeImg = () => {
	getCode({})
		.then(data => {
			const obj = data.obj;
			loginForm.value.code_key = obj.code_key; // 存储验证码key
			codeNet.value = obj.code; // 存储验证码内容
		})
		.catch(() => {
		});
};
